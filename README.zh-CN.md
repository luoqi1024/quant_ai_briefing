...
Quant AI Briefing 是一个用于个人投资影子记账和 AI 简报的小型 Python 服务。它会把定投规则、影子交易、行情和每日组合快照存入 SQLite，计算持仓成本、市值、今日盈亏和累计浮动盈亏，然后调用兼容 OpenAI Chat Completions 的接口生成 Markdown 日报或周报，并可通过企业微信自建应用推送。

本项目不会连接券商交易接口，也不会执行真实交易。它适合用于个人复盘、资产记录和组合监控。

## 功能

- 使用 SQLite 存储定投规则、影子交易、行情快照和历史组合快照。
- 支持日、周、月频率的规则触发。
- 对同一天生成的影子交易做去重保护。
- 计算持仓成本、市值、今日盈亏、累计浮动盈亏和盈亏率。
- 热门观察池的美股和境内 ETF 使用腾讯/新浪双行情源，并支持有时效限制的历史快照降级。
- 根据中国法定节假日日历，仅在工作日生成日报。
- 每周日基于已保存的周一至周五快照生成独立周报。
- 日报和周报使用不同 AI Prompt，并分别提供本地测试兜底模板。
- 支持企业微信 Markdown 推送，并按企业微信字节限制自动拆分长消息。
- 支持 `--dry-run` 本地测试，不需要真实 AI 和推送凭据。
- 支持 `--check-market-data` 行情健康检查，不调用 AI、不写投资数据、不发送通知。
...
热门观察池成功获取的公开行情会保存到 `market_snapshots`。实时源全部失败时，
日报可以使用最近七个自然日内的历史行情快照（数字货币最多一天），并明确标注
“历史快照”和报价日期；超过时效的快照不会使用。
...
日报模式在周末和中国法定节假日会直接正常退出，不抓行情、不执行影子定投、不调用 AI 或企业微信。即使周末属于调休工作日，也按设计跳过日报。周报模式不会生成影子交易，也不会重新抓取持仓实时行情，而是读取刚结束的周一至周五历史快照。

生产模式要求 AI 配置完整，AI 接口失败时本次任务会失败退出。确定性的本地兜底模板仅供 `--dry-run` 测试使用。

## 部署示例

工作日日报和周日周报均在 08:00 触发的 cron 示例：
